//! # Matomo addon
//!
//! This module provide the matomo custom resource and its definition
...
pub const ADDON_FINALIZER: &str = "api.clever-cloud.com/matomo";
...
impl Matomo {
    ...
    /// Sets the URL of the matomo instance (`METABASE_URL` secret)
    #[cfg_attr(feature = "tracing", tracing::instrument)]
    ...
    /// Returns the URL of the matomo instance (`METABASE_URL` secret)
    #[cfg_attr(feature = "tracing", tracing::instrument)]
...
impl ControllerBuilder<Matomo> for Reconciler {
    fn build(&self, state: Arc<Context>) -> Controller<Matomo> {
...
impl k8s::Reconciler<Matomo> for Reconciler {
    type Error = ReconcilerError;

    async fn upsert(ctx: Arc<Context>, origin: Arc<Matomo>) -> Result<(), ReconcilerError> {
        let Context {
            kube,
            apis,
            config: _,
        ...
        let kind = Matomo::kind(&()).to_string();
        ...
        let url = match &secrets {
            None => None,
            Some(secrets) => match secrets.get("MATOMO_URL") {
    ...
    async fn delete(ctx: Arc<Context>, origin: Arc<Matomo>) -> Result<(), ReconcilerError> {
        let Context {
            apis,
            kube,
            config: _,
