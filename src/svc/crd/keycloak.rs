//! # Keycloak addon
//!
//! This module provide the keycloak custom resource and its definition
...
pub const ADDON_FINALIZER: &str = "api.clever-cloud.com/keycloak";
...
#[kube(version = "v1")]
#[kube(kind = "Keycloak")]
#[kube(singular = "keycloak")]
#[kube(plural = "keycloaks")]
#[kube(shortname = "kc")]
#[kube(status = "Status")]
#[kube(namespaced)]
...
#[kube(
    printcolumn = r#"{"name":"admin_url", "type":"string", "description":"Admin URL", "jsonPath":".status.admin_url"}"#
)]
pub struct Spec {
...
impl Keycloak {
    ...
    /// Sets the URL of the metabase instance (`CC_KEYCLOAK_URL` secret)
    #[cfg_attr(feature = "tracing", tracing::instrument)]
    ...
    /// Returns the URL of the metabase instance (`CC_KEYCLOAK_URL` secret)
    #[cfg_attr(feature = "tracing", tracing::instrument)]
    pub fn get_url(&self) -> Option<String> {
        self.status.to_owned().unwrap_or_default().url
    }

    /// Sets the admin URL of the metabase instance (`CC_KEYCLOAK_ADMIN_URL` secret)
    #[cfg_attr(feature = "tracing", tracing::instrument)]
    pub fn set_admin_url(&mut self, admin_url: Option<String>) {
        let status = self.status.get_or_insert_with(Status::default);
    ...
    /// Returns the admin URL of the metabase instance (`CC_KEYCLOAK_ADMIN_URL` secret)
    #[cfg_attr(feature = "tracing", tracing::instrument)]
    pub fn get_admin_url(&self) -> Option<String> {
        self.status.to_owned().unwrap_or_default().admin_url
    }
}
...
impl Display for Action {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match self {
            ...
            Self::UpdateAdminUrl => write!(f, "UpdateAdminUrl"),
...
impl ControllerBuilder<Keycloak> for Reconciler {
    fn build(&self, state: Arc<Context>) -> Controller<Keycloak> {
...
impl k8s::Reconciler<Keycloak> for Reconciler {
    ...
    async fn upsert(ctx: Arc<Context>, origin: Arc<Keycloak>) -> Result<(), ReconcilerError> {
        let Context {
            kube,
            apis,
            config: _,
        ...
        let kind = Keycloak::kind(&()).to_string();
        ...
        let message = &format!(
            "Create managed keycloak instance on clever-cloud '{}'",
            addon.id
        );
        recorder::normal(kube.to_owned(), &modified, action, message).await?;
        ...
        let url = match &secrets {
            None => None,
            Some(secrets) => match secrets.get("CC_KEYCLOAK_URL") {
                Some(secret) if !secret.is_empty() => Some(secret.to_string()),
                _ => None,
            },
        };

        let admin_url = match &secrets {
            None => None,
            Some(secrets) => match secrets.get("CC_KEYCLOAK_ADMIN_URL") {
                Some(secret) if !secret.is_empty() => Some(secret.to_string()),
                _ => None,
            },
        };
        ...
        if let Some(admin_url) = admin_url {
            if modified.get_admin_url().as_deref() != Some(admin_url.as_str()) {
                let action = &Action::UpdateAdminUrl;
                let message = &format!("Update admin url to '{admin_url}'");

                modified.set_admin_url(Some(admin_url));
    ...
    async fn delete(ctx: Arc<Context>, origin: Arc<Keycloak>) -> Result<(), ReconcilerError> {
        let Context {
            apis,
            kube,
            config: _,
        ...
        let message = "Delete managed keycloak instance on clever-cloud";
        recorder::normal(kube.to_owned(), &modified, action, message).await?;
